...
// Database object definition first
const db = {
    ...
    users: {
        ...
        findById: async (id) => {
            if (db.isUsingMongoDB) return await User.findById(id);
            return readData('users').find(u => u._id === id);
        },
        findOne: async (query) => {
            if (db.isUsingMongoDB) return await User.findOne(query);
            return readData('users').find(u => {
                return Object.keys(query).every(key => u[key] === query[key]);
            });
        },
        create: async (userData) => {
            if (db.isUsingMongoDB) return await User.create(userData);
        ...
        findByIdAndUpdate: async (id, updates) => {
            if (db.isUsingMongoDB) return await User.findByIdAndUpdate(id, updates, { new: true });
            const users = readData('users');
    ...
    matches: {
        create: async (matchData) => {
            if (db.isUsingMongoDB) return await Match.create(matchData);
    ...
    reports: {
        create: async (reportData) => {
            if (db.isUsingMongoDB) return await Report.create(reportData);
    ...
    chats: {
        find: async (query = {}) => {
            if (db.isUsingMongoDB) return await Chat.find(query);
        ...
        findById: async (id) => {
            if (db.isUsingMongoDB) return await Chat.findById(id);
            return readData('chats').find(c => c._id === id);
        },
        findOne: async (query) => {
            if (db.isUsingMongoDB) return await Chat.findOne(query);
            const chats = readData('chats');
        ...
        create: async (chatData) => {
            if (db.isUsingMongoDB) return await Chat.create(chatData);
            const chats = readData('chats');
        ...
        findByIdAndUpdate: async (id, updates) => {
            if (db.isUsingMongoDB) return await Chat.findByIdAndUpdate(id, updates, { new: true });
            const chats = readData('chats');
